...
class SevenSegmentDisplay:
    ...
    def decode_positions(self) -> Dict[str, str]:
        ...
        positions["top_center"] = "".join(
            set.difference(self.inputs_by_len[3][0], self.inputs_by_len[2][0])
        )
            ...
            if set.difference(self.inputs_by_len[4][0], input) == set():
                positions["bottom_left"] = "".join(
                    set.difference(
                        ...
                        set.union(self.inputs_by_len[4][0], input),
                    )
                )
            if len(set.intersection(input, self.inputs_by_len[2][0])) == 1:
                positions["top_right"] = "".join(
                    set.difference(self.inputs_by_len[7][0], input)
                )

        positions["bottom_right"] = "".join(
            set.difference(self.inputs_by_len[2][0], set(positions["top_right"]))
        )
        positions["bottom_center"] = "".join(
            set.difference(
                self.inputs_by_len[7][0],
                set.union(
                    self.inputs_by_len[4][0],
                    {positions["bottom_left"], positions["top_center"]},
                ),
            )
        )
        ...
        for input in self.inputs_by_len[6]:
            if (
                len(
                    set.difference(
                        self.inputs_by_len[7][0],
                        set.union(input, set(positions.values())),
                    )
                )
                == 1
            ):
                positions["center"] = "".join(
                    set.difference(self.inputs_by_len[7][0], input)
                )

        positions["top_left"] = "".join(
            set.difference(self.inputs_by_len[7][0], set(positions.values()))
        )
    ...
    def output(self):
        ...
        for output in self.outputs:
            for number, required_segments in self.number_mapping.items():
                if (
                    set(required_segments.format_map(SafeDict(self.positions)))
                    == output
                ):
                    digit += str(number)
                    break
