...
while len(numbers_to_consider_for_oxygen_rating) > 1:
    ...
    if len(most_common_numbers_at_current_index) == 2:
        ...
        most_common = (
            "1" if most_common_count == second_most_common_count else most_common
        )
    else:
    ...
    numbers_to_consider_for_oxygen_rating = list(
        filter(
            lambda x: x[current_index] == most_common,
            numbers_to_consider_for_oxygen_rating,
        )
    )
    current_index += 1
...
while len(numbers_to_consider_for_co2_scrubber_rating) > 1:
    least_common_numbers_at_current_index = sorted(
        Counter(
            map(lambda x: x[current_index], numbers_to_consider_for_co2_scrubber_rating)
        ).most_common(),
        key=lambda x: x[1],
    )
    ...
    if len(least_common_numbers_at_current_index) == 2:
        ...
        least_common = (
            "0" if least_common_count == second_least_common_count else least_common
        )
    else:
    ...
    numbers_to_consider_for_co2_scrubber_rating = list(
        filter(
            lambda x: x[current_index] == least_common,
            numbers_to_consider_for_co2_scrubber_rating,
        )
    )
    current_index += 1
