...
class DumboOctopus:
    ...
    def get_adjacent_octopuses(self) -> List[DumboOctopus]:
        ...
        return [
            self.coordinate_to_octopus[coordinate]
            for coordinate in candidate_coordinates
        ]
...
class DumboOctopuses:
    ...
    def __init__(self, octopuses: List[DumboOctopus]) -> None:
        ...
        coordinate_to_octopus = {
            (octopus.x, octopus.y): octopus for octopus in self.octopuses
        }
    ...
    def simulate_step(self):
        ...
        if all(octopus.has_flashed for octopus in self.octopuses):
            DumboOctopuses.ALL_FLASHED = True
...
def main() -> None:
    ...
    current_step = 0
    while not DumboOctopuses.ALL_FLASHED:
